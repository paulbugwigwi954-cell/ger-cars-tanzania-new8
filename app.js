...
async function toggleFavourite(id){
 if(!authUser){
   if(favourites.includes(id)) favourites=favourites.filter(x=>x!==id); else favourites.push(id);
   localStorage.setItem('ger_favourites',JSON.stringify(favourites));renderCars();renderFavs();toast(favourites.includes(id)?'Car saved locally. Login to sync it to your account.':'Car removed');return;
 }
 try{
   if(favourites.includes(id)){
     const {error}=await supabaseClient.from('favourites').delete().eq('user_id',authUser.id).eq('car_id',id);if(error)throw error;
     favourites=favourites.filter(x=>x!==id);toast('Removed from your favourites');
   }else{
     const {error}=await supabaseClient.from('favourites').insert({user_id:authUser.id,car_id:id});if(error)throw error;
     favourites.push(id);toast('Saved to your account');
   }
...
async function submitCarEnquiry(e,c){e.preventDefault();if(!authUser){toast('Please login to send an enquiry');authModal('login');return}const f=new FormData(e.target);const btn=e.target.querySelector('button');btn.disabled=true;try{const {error}=await supabaseClient.from('enquiries').insert({car_id:c.id,customer_id:authUser.id,customer_name:String(f.get('name')||''),customer_phone:String(f.get('phone')||''),customer_email:String(f.get('email')||authUser.email||''),message:String(f.get('message')||''),channel:'website',status:'new'});if(error)throw error;closeModal();toast('Enquiry sent successfully');}catch(err){toast(err.message||'Could not send enquiry')}finally{btn.disabled=false}}
...
async function openAccount(){
 ...
 openModal(`<span class="eyebrow">MY ACCOUNT</span><h2>${profile.full_name||authUser.email}</h2><form id="profileForm" class="contact-form"><input name="full_name" required placeholder="Full name" value="${profile.full_name||''}"><input name="phone" placeholder="Phone number" value="${profile.phone||''}"><input value="${authUser.email||''}" disabled><button class="btn primary">Save Profile</button></form><hr><div class="account-section"><h3>My Car Listings</h3><p class="muted">Manage your own listings. A sold car can be permanently deleted.</p><div>${listingRows}</div></div><hr><h3>My Enquiries</h3><div>${rows}</div>${profile.role==='admin'?'<button id="adminReviewBtn" class="btn primary" style="margin-top:16px">Admin: Review Listings</button>':''}<button id="accountLogout" class="btn ghost" style="margin-top:16px">Logout</button>`);
...
async function updateAuthUI(){if(!supabaseClient)return;const {data}=await supabaseClient.auth.getSession();authUser=data.session?.user||null;if(authUser){await loadProfile();await loadFavourites();const btn=$('#loginBtn');btn.textContent='My Account';btn.onclick=openAccount}else{profile=null;const btn=$('#loginBtn');btn.textContent='Login';btn.onclick=()=>authModal('login');await loadFavourites()}}
if(supabaseClient){supabaseClient.auth.onAuthStateChange(async()=>{setTimeout(updateAuthUI,0)})}
$('#searchBtn').onclick=applyFilters;$('#sortSelect').onchange=()=>{sortCars();renderCars()};$('#clearBtn').onclick=()=>{$('#searchInput').value='';$('#makeFilter').value='';$('#conditionFilter').value='';$('#locationFilter').value='';applyFilters()};$$('[data-filter]').forEach(a=>a.onclick=()=>{setTimeout(()=>{$('#conditionFilter').value=a.dataset.filter;applyFilters()},0)});$$('[data-cat]').forEach(b=>b.onclick=()=>{const cat=b.dataset.cat;current=CARS.filter(c=>c.body===cat);renderCars();location.hash='cars'});$('#sellBtn').onclick=openSell;$('#modalClose').onclick=closeModal;$('#modal').onclick=e=>{if(e.target.id==='modal')closeModal()};$('#contactForm').onsubmit=async e=>{e.preventDefault();if(!authUser){toast('Please login to send your enquiry');authModal('login');return}const f=new FormData(e.target);const {error}=await supabaseClient.from('enquiries').insert({customer_id:authUser.id,customer_name:String(f.get('name')||''),customer_phone:String(f.get('phone')||''),customer_email:String(f.get('email')||authUser.email||''),message:String(f.get('message')||''),channel:'website',status:'new'});if(error)toast(error.message);else{e.target.reset();toast('Enquiry sent successfully')}};$('#menuBtn').onclick=()=>$('#mainNav').classList.toggle('open');
