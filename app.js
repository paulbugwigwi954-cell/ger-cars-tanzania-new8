...
function openSell(){
 ...
 videoInput.onchange=async()=>{videoPreview.innerHTML='';const file=videoInput.files?.[0];if(!file)return;try{if(!['video/mp4','video/webm','video/quicktime','video/x-m4v'].includes(file.type))throw new Error('Video must be MP4, WebM or MOV.');if(file.size>100*1024*1024)throw new Error('Video must be 100 MB or smaller.');const duration=await getVideoDuration(file);if(duration>60.5)throw new Error('Dealer video must not exceed 1 minute.');const box=document.createElement('div');box.className='photo-upload-item';const v=document.createElement('video');v.controls=true;v.preload='metadata';v.src=URL.createObjectURL(file);box.appendChild(v);const n=document.createElement('span');n.textContent=Math.round(duration)+'s';box.appendChild(n);videoPreview.appendChild(box)}catch(err){toast(err.message);videoInput.value='';}};
 photoInput.onchange=()=>{
