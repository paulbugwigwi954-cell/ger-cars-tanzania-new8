...
function carVideo(c){return CAR_VIDEOS[c.id]||null}
...
function carCard(c){
 const fav=favourites.includes(c.id),cmp=compare.includes(c.id),imgs=carImages(c),video=carVideo(c);
 const photoBlock=imgs.length
  ? `<div class="car-photo"><img class="car-main-image" data-main-image="${c.id}" src="${imgs[0]}" alt="${c.make} ${c.model}" loading="lazy"><span class="badge">${c.featured?'FEATURED':'GER VERIFIED'}</span><button class="fav ${fav?'active':''}" data-fav="${c.id}" aria-label="Favourite">${fav?'♥':'♡'}</button></div><div class="photo-strip">${imgs.map((src,i)=>`<button class="photo-thumb ${i===0?'active':''}" data-photo-target="${c.id}" data-photo="${src}"><img src="${src}" alt="${c.make} ${c.model} photo ${i+1}" loading="lazy"></button>`).join('')}<span class="photo-count">${imgs.length} photos</span></div>`
  : `<div class="car-photo" style="display:flex;align-items:center;justify-content:center;min-height:220px;background:#f1f3f5"><span class="muted">No dealer photo uploaded</span><span class="badge">${c.featured?'FEATURED':'GER VERIFIED'}</span><button class="fav ${fav?'active':''}" data-fav="${c.id}" aria-label="Favourite">${fav?'♥':'♡'}</button></div>`;
 return `<article class="car-card">${photoBlock}${video?`<div class="dealer-video-card"><video controls controlsList="nodownload noplaybackrate" disablePictureInPicture preload="metadata" playsinline oncontextmenu="return false" src="${video.public_url}"></video><div class="video-indicator">▶ Dealer video · ${Math.round(Number(video.duration_seconds||0))} sec</div></div>`:''}<div class="car-info"><div class="car-title"><h3>${c.make} ${c.model}</h3><span class="price">${money(c.price)}</span></div><div class="meta"><span>${c.year}</span><span>${Number(c.mileage||0).toLocaleString()} KM</span><span>${c.transmission||'—'}</span><span>${c.fuel||'—'}</span><span>${c.location||'—'}</span></div><div class="card-actions"><button class="btn ghost" data-detail="${c.id}">View Details</button><button class="btn compare-btn" data-compare="${c.id}">${cmp?'✓ Compared':'Compare'}</button></div></div></article>`
}
...
async function loadCarVideos(){
 if(!supabaseClient || !CARS.length){CAR_VIDEOS={};renderCars();renderFavs();renderCompare();return}
 const ids=CARS.map(c=>c.id);const {data,error}=await supabaseClient.from('car_videos').select('car_id,public_url,duration_seconds').in('car_id',ids);
 if(error){toast(error.message||'Could not load dealer videos');return}
 CAR_VIDEOS={};(data||[]).forEach(v=>{if(v.public_url)CAR_VIDEOS[v.car_id]=v});renderCars();renderFavs();renderCompare();
}
...
async function createListing(form){
 ...
 try{
   ...
   if(videoFile){if(!['video/mp4','video/webm','video/quicktime','video/x-m4v'].includes(videoFile.type))throw new Error('Video must be MP4, WebM or MOV.');if(videoFile.size>100*1024*1024)throw new Error('Video must be 100 MB or smaller.');const duration=await getVideoDuration(videoFile);if(!Number.isFinite(duration)||duration>60)throw new Error('Dealer video must not exceed 1 minute.');}
...
function openSell(){
 ...
 const mediaRadios=[...document.querySelectorAll('input[name="mediaChoice"]')];
 const updateMediaChoice=()=>{const choice=document.querySelector('input[name="mediaChoice"]:checked')?.value||'photos';const photoLabel=photoInput.closest('.file-label'),videoLabel=videoInput.closest('.file-label');if(photoLabel)photoLabel.style.display=choice==='photos'?'block':'none';if(videoLabel)videoLabel.style.display=choice==='video'?'block':'none';if(choice==='photos'){videoInput.value='';videoPreview.innerHTML='';}else{photoInput.value='';preview.innerHTML='';}};
 const videoInput=$('#carVideoInput'),videoPreview=$('#videoPreview');
 mediaRadios.forEach(r=>r.onchange=updateMediaChoice);
 updateMediaChoice();
 videoPreview.onclick=e=>{if(e.target.closest('#removeVideoBtn')){videoInput.value='';videoPreview.innerHTML='';toast('Video removed. You can choose another video.')}};
 videoInput.onchange=async()=>{videoPreview.innerHTML='';const file=videoInput.files?.[0];if(!file)return;try{if(!['video/mp4','video/webm','video/quicktime','video/x-m4v'].includes(file.type))throw new Error('Video must be MP4, WebM or MOV.');if(file.size>100*1024*1024)throw new Error('Video must be 100 MB or smaller.');const duration=await getVideoDuration(file);if(duration>60)throw new Error('Dealer video must not exceed 1 minute.');const box=document.createElement('div');box.className='photo-upload-item';const v=document.createElement('video');v.controls=true;v.controlsList='nodownload noplaybackrate';v.disablePictureInPicture=true;v.preload='metadata';v.src=URL.createObjectURL(file);box.appendChild(v);const n=document.createElement('span');n.textContent=Math.round(duration)+'s';box.appendChild(n);const remove=document.createElement('button');remove.type='button';remove.className='photo-remove-btn';remove.textContent='×';remove.title='Remove this video';remove.id='removeVideoBtn';box.appendChild(remove);videoPreview.appendChild(box)}catch(err){toast(err.message);videoInput.value='';}};
 photoInput.onchange=()=>{
