...
function carCard(c){
 ...
 return `<article class="car-card">${photoBlock}${video?`<div class="dealer-video-card"><video controls preload="metadata" playsinline src="${video.public_url}"></video><div class="video-indicator">▶ Dealer video · ${Math.round(Number(video.duration_seconds||0))} sec</div></div>`:''}<div class="car-info"><div class="car-title"><h3>${c.make} ${c.model}</h3><span class="price">${money(c.price)}</span></div><div class="meta"><span>${c.year}</span><span>${Number(c.mileage||0).toLocaleString()} KM</span><span>${c.transmission||'—'}</span><span>${c.fuel||'—'}</span><span>${c.location||'—'}</span></div><div class="card-actions"><button class="btn ghost" data-detail="${c.id}">View Details</button><button class="btn compare-btn" data-compare="${c.id}">${cmp?'✓ Compared':'Compare'}</button></div></div></article>`
}
...
async function createListing(form){
 ...
 try{
   ...
   if(videoFile){if(!['video/mp4','video/webm','video/quicktime','video/x-m4v'].includes(videoFile.type))throw new Error('Video must be MP4, WebM or MOV.');if(videoFile.size>100*1024*1024)throw new Error('Video must be 100 MB or smaller.');const duration=await getVideoDuration(videoFile);if(!Number.isFinite(duration)||duration>120)throw new Error('Dealer video must not exceed 2 minutes.');}
...
function openSell(){
 ...
 videoInput.onchange=async()=>{videoPreview.innerHTML='';const file=videoInput.files?.[0];if(!file)return;try{if(!['video/mp4','video/webm','video/quicktime','video/x-m4v'].includes(file.type))throw new Error('Video must be MP4, WebM or MOV.');if(file.size>100*1024*1024)throw new Error('Video must be 100 MB or smaller.');const duration=await getVideoDuration(file);if(duration>120)throw new Error('Dealer video must not exceed 2 minutes.');const box=document.createElement('div');box.className='photo-upload-item';const v=document.createElement('video');v.controls=true;v.preload='metadata';v.src=URL.createObjectURL(file);box.appendChild(v);const n=document.createElement('span');n.textContent=Math.round(duration)+'s';box.appendChild(n);const remove=document.createElement('button');remove.type='button';remove.className='photo-remove-btn';remove.textContent='×';remove.title='Remove this video';remove.id='removeVideoBtn';box.appendChild(remove);videoPreview.appendChild(box)}catch(err){toast(err.message);videoInput.value='';}};
 photoInput.onchange=()=>{
