...
async function toggleFavourite(id){
 if(!authUser){toast('Please login first to change your favourites.');authModal('login');return}
...
function bindCards(){
 ...
 $('[data-compare]').forEach(b=>b.onclick=()=>{if(!authUser){toast('Please login first to use Compare.');authModal('login');return}const id=b.dataset.compare;if(compare.includes(id))compare=compare.filter(x=>x!==id);else if(compare.length<4)compare.push(id);else return toast('Compare limit is 4 cars');localStorage.setItem('ger_compare',JSON.stringify(compare));navCounts();renderCars();renderCompare()});
